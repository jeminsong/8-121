...
TEST(CarClass, AccessorsAndMutators) {
  ...
  ASSERT_EQ((your_car.GetId()).GetModel(), unittest_identity.GetModel())
    << "The car's VehicleId model should be Ford";
  ASSERT_EQ((your_car.GetId()).GetVin(), unittest_identity.GetVin())
    << "The car's VehicleId VIN should be 1";
  ASSERT_EQ((your_car.GetId()).GetLicensePlate(),
            unittest_identity.GetLicensePlate())
    << "The car's VehicleId license plate should be 123456";
  ASSERT_EQ((your_car.GetReleaseDate()).GetDay(), unittest_date.GetDay())
    << "The car's release date day should be 28";
  ASSERT_EQ((your_car.GetReleaseDate()).GetMonth(), unittest_date.GetMonth())
    << "The car's release date month should be 3";
  ASSERT_EQ((your_car.GetReleaseDate()).GetYear(), unittest_date.GetYear())
    << "The car's release date year should be 1984";
}

TEST(CarClass, DefaultConstructor) {
  Car your_car;
  ASSERT_EQ((your_car.GetId()).GetModel(), "Tesla")
    << "The car's VehicleId model should be Tesla";
  ASSERT_EQ((your_car.GetId()).GetVin(), 121)
    << "The car's VehicleId VIN should be 121";
  ASSERT_EQ((your_car.GetId()).GetLicensePlate(), "TUFFY121L")
    << "The car's VehicleId license plate should be TUFFY121L";
  ASSERT_EQ((your_car.GetReleaseDate()).GetDay(), 1)
    << "The car's release date day should be 1";
  ASSERT_EQ((your_car.GetReleaseDate()).GetMonth(), 1)
    << "The car's release date month should be 1";
  ASSERT_EQ((your_car.GetReleaseDate()).GetYear(), 2022)
...
TEST(CarClass, NonDefaultVehicleIdConstructor) {
  ...
  ASSERT_EQ((your_car.GetId()).GetModel(), "Honda")
    << "The car's VehicleId model should be Honda";
  ASSERT_EQ((your_car.GetId()).GetVin(), 3)
    << "The car's VehicleId VIN should be 3";
  ASSERT_EQ((your_car.GetId()).GetLicensePlate(), "7B319X4")
...
TEST(CarClass, NonDefaultReleaseDateConstructor) {
  ...
  ASSERT_EQ((your_car.GetId()).GetLicensePlate(), "TUFFY121L")
    << "The car's VehicleId license plate should be TUFFY121L";
  ASSERT_EQ((your_car.GetReleaseDate()).GetDay(), 28)
    << "The car's release date day should be 28";
  ASSERT_EQ((your_car.GetReleaseDate()).GetMonth(), 3)
    << "The car's release date month should be 3";
  ASSERT_EQ((your_car.GetReleaseDate()).GetYear(), 1984)
    << "The car's release date year should be 1984";
}
